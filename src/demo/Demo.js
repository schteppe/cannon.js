...
CANNON.Demo = function(options){
	...
	if(settings.contacts){
		...
		for(var ij=0; ij < 2; ij++){
		    ...
		    mesh.position.x = b.position.x + r.x;
		    mesh.position.y = b.position.y + r.y;
		    mesh.position.z = b.position.z + r.z;
		}
	    }
	}
	contactMeshCache.hideCached();
	...
	rf.add(settings,'contacts').onChange(function(contacts){
	    // Do nothing... contacts are dynamically added/removed for each frame
	});
	rf.add(settings,'cm2contact').onChange(function(cm2contact){});
	rf.add(settings,'normals').onChange(function(normals){});
	rf.add(settings,'constraints').onChange(function(constraints){});
	rf.add(settings,'axes').onChange(function(axes){});
	...
	if(body instanceof CANNON.RigidBody)
	    ...
	    mesh.scale.set(settings.particleSize,
